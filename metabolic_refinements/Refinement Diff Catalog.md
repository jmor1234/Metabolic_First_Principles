...
**Coverage:** This catalog contains every file/passage-level diff drafted from the stress test so far — Layer 1 (Sub-tests 1.A and 1.B) and Layer 2.A. Layer 2.B (lactate-active claim / Loop 9 / histone lactylation causality) has been deferred pending a 13-loop causal-independence audit that uses the new audit tool derived from Sub-test 2.A; Layers 3–5 have not started. The catalog will continue to grow as later stress-test layers produce additional refinements.

**Note on methodology improvements.** This catalog tracks *claim-level* diffs — refinements to specific passages in `metabolic_foundations/` that are preserved here as parallel knowledge because the decision was made not to overwrite the original vault. *Methodology improvements* are a different category and are not staged here. Two methodology improvements have been applied directly to `metabolic_foundations/Verification Methodology.md` as edits rather than as parallel-knowledge diffs: **CF-2.A.4** (citation deployment audit, added to Step 5 on 2026-04-10) and **the causal-independence audit** (four-check operational audit grounded in Pearl's screening-off test and MCA conservation relations, added to Step 4 on 2026-04-11). Both are derived from Sub-test 2.A. They are tracked in `Stress Test.md`'s working log entries and in `Current State.md`'s Active Decisions section. The refined causal-independence audit also has a standalone document in this vault: `Causal Independence in Feedback Loops.md`.
