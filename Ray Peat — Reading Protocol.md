...
- **Don't default to mainstream consensus as the baseline.** Mainstream consensus is one data point among several — not the arbiter. The arbiter is first-principles reasoning from primary evidence. When reasoning gets hard, the instinct to retreat to "consensus view" feels neutral but often reproduces paradigm-captured framings that the vault and Peat are specifically designed to stress-test. Peat, the vault, and mainstream consensus are three independent sources to weigh — none gets automatic deference. Watch for the drift signs in your own writing: "bidirectional," "both sides have a point," "the picture is complicated," "current consensus is" — reaching for these phrases when the evidence actually favors one direction is the retreat to consensus. First-principles here means: thermodynamics (cells require ATP; every function is energy-dependent), stoichiometry (what is actually co-produced, in what ratios, with what coupling), mechanism specificity (what molecular binding sites, what specific pathway), and falsifiability (what would disprove this claim). Use those standards, use primary literature as evidence, use the vault as the current best integration — and reason. Consensus is an input, never a conclusion.
...
**Trace summaries to mechanism documents before declaring a gap.** Summary statements in `CLAUDE.md` files and Historical Assessment documents are compressed. They can appear global when the actual mechanism document scope-limits the claim explicitly. Canonical examples observed in practice:
- `metabolic_framework/CLAUDE.md` line 150 states "CO2 is a parallel marker, not a causal driver in any feedback loop" — correct but compressed. `The Metabolic Fork.md` lines 166-167 contain a dedicated `[!warning]` callout "CO₂ Is a Parallel Marker in This Framework's Feedback Loops — Not Everywhere" naming sAC, Cx26, and ubiquitin K48 as independent-driver exceptions. `Verification Methodology.md` lines 239-241 have a worked counterexample making the same point.
- Cardiolipin / PUFA / linoleate question: `Membrane Damage and Defense.md` line 62 explicitly states *"The question is not whether PUFA in cardiolipin is required for function — it is not — but whether the PUFA-rich composition created by the modern diet makes the membrane more vulnerable to peroxidative damage over a lifetime,"* citing the yeast cardiolipin example as direct evidence. A surface-level reading of "PUFA bad" / "reduce PUFA burden" misses this sophisticated framing entirely.

**The base-rate expectation when you think you have a refinement candidate.** When you believe you've identified a vault gap, the base rate is that **the vault already has the insight at the mechanism-document level in a form more sophisticated than your summary-level reading caught**. This is not pessimism about investigation — it is accurate calibration to what this vault actually contains. The refinement rate is low because the vault was built through rigorous stress-testing and incorporates sophisticated framings throughout. Before proposing anything, verify specifically against the mechanism document prose. Hypothesis 1 of the Verification Methodology (the vault is being misunderstood) applies especially here: surface reading of CLAUDE.md summaries produces false refinement candidates that evaporate when traced to the mechanism document. If the mechanism document genuinely doesn't address the specific case, then consider whether it's an additive expansion rather than a corrective refinement.
